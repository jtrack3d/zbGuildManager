...
ZbGm.ZRoster = {
	...
	AddCharacter = function(self, parentIndex, name, server, rank, class, note, officernote, login, join) 
		...
		if self.players[myIndex] == nil then
			...
			--print(self.players[myIndex].parentIndex)
		else
	...
	BuildFilteredIndex = function(self, searchfor, mains) 
	...
	BuildUnassociatedList = function(self) 
		local unassoc = nil;
		for idx in pairs(self.players) do
	...
	GetFilteredCharacter = function(self, index)
		return ZbGm.ZRoster.players[ZbGm.ZRoster.playerIndex[index]];
	end,
	
	GetCharacter = function(self, charIndex)
	...
	GetCharacterIndices = function(self, character, charTable)
	...
	GetJoinDate = function(self, node) 
		if node.parentNode then
	...
	GetLastLogin = function(self, node)
		if node then 
			if node.parentNode then
	...
	GetFilteredIndexCount = function(self)
        return #(self.playerIndex)
	end,
	
	ReconstructLists = function(self, savedDatesDB) 
	...
	InsertChildNode = function(self, node, parentNode)
	...
	RemoveChildNode = function(self, node) 
	...
	RemoveCharacter = function(self, indexName)
	...
	SortIndex = function(self)
		table.sort(self.playerIndex, self.sortByFunctions[self.sortIndexBy])
	end,	
	
	SwapMain = function(self, indexOldMain, indexNewMain)
	...
	UpdateHistorySave = function(self, memberDB) 
	...
	DumpPlayer = function(self, mainIndex)
		-- Get a node.
